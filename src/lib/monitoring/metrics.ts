...
export class MetricsCollector {
  ...
  public getAggregatedMetrics(): Record<string, any> {
    ...
    for (const [endpoint, perfData] of Array.from(this.performanceData.entries())) {
  ...
  public getHealthData(): Record<string, any> {
    ...
    for (const perfData of Array.from(this.performanceData.values())) {
      if (perfData.length > 0) {
...
export function withMetrics(
  ...
  return async (req: NextRequest, context?: any): Promise<NextResponse> => {
    ...
    try {
      ...
      // Increment error counter
      metricsCollector.incrementError(endpoint, error instanceof Error ? error.constructor.name : 'UnknownError');
