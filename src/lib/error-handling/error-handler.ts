...
export class ErrorHandler {
   ...
   */
  private normalizeError(error: Error | BaseAppError, context?: ErrorContext): BaseAppError {
    ...
    if (error.name === 'ValidationError') {
      return new ValidationError(error instanceof Error ? error.message : 'Validation error', context);
    }
    
    if (error.name === 'UnauthorizedError') {
      return new AuthenticationError(error instanceof Error ? error.message : 'Authentication error', context);
    }
    
    if (error.name === 'ForbiddenError') {
      return new AuthorizationError(error instanceof Error ? error.message : 'Authorization error', context);
    }
    
    if (error.name === 'NotFoundError') {
      return new NotFoundError(error instanceof Error ? error.message : 'Not found error', context);
    }
    
    if (error.name === 'RateLimitError') {
      return new RateLimitError(error instanceof Error ? error.message : 'Rate limit error', context);
    }
   ...
   */
  private async logError(error: BaseAppError, request?: NextRequest): Promise<void> {
    const logData = {
      error: {
        code: error.code,
   ...
   */
  private trackError(error: BaseAppError): void {
   ...
   */
  private async checkForAlerts(error: BaseAppError): Promise<void> {
   ...
   */
  private async triggerAlert(error: BaseAppError, count: number): Promise<void> {
   ...
   */
  private createErrorResponse(error: BaseAppError): NextResponse {
    const responseData = {
      success: false,
      error: {
        code: error.code,
...
export function createStandardErrorResponse(
  ...
  // Create a new error object with the correct properties
  const customError = {
    ...error,
    code,
    statusCode,
  ...
  const responseData = {
    ...
    timestamp: customError.timestamp.toISOString(),
